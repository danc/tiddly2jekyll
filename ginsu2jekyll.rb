...
Splitter2Jekyll.quiet = options.quiet
Splitter2Jekyll.usesubdirectories = options.usesubdirectories
Splitter2Jekyll.tagsubdirectory = options.tagsubdirectory
Tiddler.ginsu = true

ARGV.each do |file|
	splitter = Splitter2Jekyll.new(file, options.dest, options.charset)
	splitter.split
end
